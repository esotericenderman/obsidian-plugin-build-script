    ...
    case "${obsidian_plugin_build_strategies[$name]}" in
        "excalidraw")
            ...
            pushd "./MathjaxToSVG" > /dev/null || {
              echo "Error: failed to access Excalidraw sub-directory!"; exit 7
            }

            npm install || {
              echo "Error: failed to install Excalidraw sub-directory dependencies!"; exit 6
            }

            npm run build || {
              echo "Error: failed to install Excalidraw sub-project!"; exit 7
            }

            popd > /dev/null

            npm run build || {
              echo "Error: failed to build Excalidraw!"; exit 7
            }
            ;;
        "obsidian-dev-utils")
